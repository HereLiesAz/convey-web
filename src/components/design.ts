 ...
 * directly by `test/design.test.ts` — it does not depend on the custom element below. The
 * element takes the block's available width as an explicit `fullWidthSp` property (in the same
 * approximate "advance-width units" the solver uses) rather than measuring the actual rendered
 * width of its content — real glyph metrics via `Canvas2D`'s `measureText` (or a `ResizeObserver`
 * against real DOM layout) are documented future work, not yet done here. Condensation is
 * rendered as a horizontal `transform: scaleX()` rather than a variable font's `wdth` axis, since
 * `ConveyType`'s Azrienoch integration (`tokens/type.ts`) does not yet expose per-line variation
 * through this component — wiring `fontVariationSettings` through is the natural next step.
 *
...
export interface ConveyDesignLine {
  ...
  /** See the file doc comment's "Motion" section. Defaults to `'none'`. */
...
/** Pure, dependency-free solver math for §11.2–§11.6 of the Design Block spec. */
export const ConveyDesignSolver: ConveyDesignSolverApi = {
  ...
  solveToWidth(
    ...
    const minCondensation = opts.minCondensation ?? 62.5
...
export class ConveyDesignElement extends HTMLElement {
  ...
  #render(): void {
    ...
    for (const solvedLine of solved) {
      const row = document.createElement('div')
      row.className = 'row'
      row.style.justifyContent = BOX_ALIGN[solvedLine.line.alignment ?? 'left']

      const el = this.#createTextElement(solvedLine)
      row.appendChild(el)
      this.#container.appendChild(row)
    }
  }

  #createTextElement(solvedLine: ConveyDesignSolvedLine): HTMLElement {
    ...
    const el: HTMLElement = isRegistered ? document.createElement(tagName as string) : document.createElement('span')
    ...
    el.style.fontWeight = String(Math.round(clamp(solvedLine.axes.weight, 1, 1000)))
    el.style.letterSpacing = `${solvedLine.axes.trackingSp}px`
    el.style.transform = `scaleX(${solvedLine.axes.condensation / 100})`
    el.style.transformOrigin = solvedLine.line.alignment === 'right' ? 'right center' : solvedLine.line.alignment === 'center' ? 'center center' : 'left center'
    return el
  }
}
